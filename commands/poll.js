...
exports.run = (client, message, args) => {
  ...
  let embed = new Discord.MessageEmbed()
    ...
    .setTitle("Polltime")
    .setDescription(
      `${message.author.username} iniciou uma votação! ` +
      "<a:biximfofo:764138279464599562>"
    )
    .addField("asunto", asunto)
    .addField(
      "utilize",
      `para sim <:tickYes:764151584145408023> /  para não <:tickNo:764151560368291860>`
    );

  message.channel.send(embed).then((m) => {
    m.react("764151584145408023").then((m2) => {
      m2.react("764151560368291860");
    });
    message.delete();
  });
};
