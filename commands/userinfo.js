...
//const bot = `\`${client.users.cache.get(id).bot}\``
...
const bot = user.bot ? '<:botTag:230105988211015680>' : '<:discord:314003252830011395>'
const title = ` ${bot} ${status[user.presence.status]} ${user.username}#${user.discriminator}`
const created_Date = `\`${formatDate('DD/MM/YYYY às HH:mm:ss', user.createdAt)}\``
...
let embedBot = new Discord.MessageEmbed()
  .setThumbnail(avatar)
  .setTimestamp()
  .setTitle(`${bot} <:online2:464520569975603200> ${user.username}#${user.discriminator}`)
  ...
  .setFooter(`comando utilizado `, message.author.displayAvatarURL({ format: "png" }));
...
/*`
                :warning: **ERROR**
<:xmark:314349398824058880> Oops ${message.author} desculpe mas \`${id}\` não é um ID valido! =/ 

need = \`()\`
optional = \`{}\`

.addField(':video_game: Jogando', `${user.presence.game ? `${user.presence.game.name}` : ' Nada'}`, inline, true)
